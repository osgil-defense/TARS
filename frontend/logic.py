...
# NOTE: this is the main application logic, it can be considered as the main function as well
def main_app():
    ...
    if not st.session_state["submitted"] and not st.session_state["run_agent"]:
        with st.form("my_form"):
            st.session_state["website"] = st.text_input(
                "What's Your Target Website/Network Address?",
                st.session_state["website"],
            )
            text = st.text_area(
                "What Cybersecurity-Related Task Do You Want To Do?", ""
            )
